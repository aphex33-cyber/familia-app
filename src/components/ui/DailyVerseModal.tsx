 ...
 *
 * ARQUITECTURA: Estilos 100% inline + createPortal → document.body.
 * Esto garantiza que NINGÚN z-index del sidebar, layout o CSS externo
 * pueda interferir con el centrado y la visibilidad del modal.
 */
...
export default function DailyVerseModal({ memberName, onClose }: DailyVerseModalProps) {
  ...
  // ─── Estilos 100% inline ─────────────────────────────────────────────────────
  // Ningún CSS externo (sidebar, layout, etc.) puede sobrescribir estilos inline.
  // z-index: 999999 garantiza que esté por encima de absolutamente todo.
  ...
  const cardStyle: CSSProperties = {
    ...
    width: '100%',
    maxWidth: '440px',
    background: 'linear-gradient(145deg, #1e1e38 0%, #12122a 100%)',
    border: '1px solid rgba(255, 210, 100, 0.35)',
    borderRadius: '24px',
    padding: '2.5rem 2rem',
    textAlign: 'center',
    boxShadow: '0 0 0 1px rgba(255,210,100,0.1), 0 0 60px rgba(255,210,100,0.12), 0 32px 80px rgba(0,0,0,0.8)',
  ...
  const glowStyle: CSSProperties = {
    ...
    right: 0,
    bottom: 0,
    background: 'radial-gradient(ellipse at 50% 0%, rgba(255,210,100,0.09) 0%, rgba(108,99,255,0.06) 50%, transparent 75%)',
    pointerEvents: 'none',
  };
  ...
  const iconStyle: CSSProperties = {
    ...
    marginBottom: '1rem',
    display: 'block',
    filter: 'drop-shadow(0 0 16px rgba(255, 210, 100, 0.55))',
  };
  ...
  const dividerStyle: CSSProperties = {
    ...
    background: 'linear-gradient(90deg, transparent, rgba(255,210,100,0.65), transparent)',
    borderRadius: '2px',
  };
  ...
  const btnStyle: CSSProperties = {
    ...
    borderRadius: '9999px',
    background: 'linear-gradient(135deg, rgba(255,210,100,0.2), rgba(255,210,100,0.07))',
    ...
    >
      <div style={cardStyle} onClick={e => e.stopPropagation()}>
        ...
        <p style={greetingStyle}>
          Buenos días,{' '}
          <strong style={{ color: '#f0f0ff', fontWeight: 700 }}>{memberName}</strong>{' '}
          🙌
        </p>
        ...
        {/* Referencia */}
        <cite style={refStyle}>— {verse.ref} · Reina-Valera 1960</cite>
        ...
        <button
          ...
          onMouseEnter={e => {
            ...
            btn.style.background = 'linear-gradient(135deg, rgba(255,210,100,0.3), rgba(255,210,100,0.12))';
            btn.style.transform = 'translateY(-2px)';
            btn.style.boxShadow = '0 6px 24px rgba(255,210,100,0.22)';
          }}
          onMouseLeave={e => {
            const btn = e.currentTarget as HTMLButtonElement;
            btn.style.background = 'linear-gradient(135deg, rgba(255,210,100,0.2), rgba(255,210,100,0.07))';
